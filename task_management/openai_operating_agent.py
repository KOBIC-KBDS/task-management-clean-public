...
For study/meeting/event lifecycles, prefer a stable workflow root for the actual named event over a narrow planning step such as review discussion, schedule decision, or prep meeting. Post-event deliverables such as follow-up materials, minutes, result sharing, completion reports, or outbound email should attach to the workflow root or nearest active workflow ancestor, not to a completed schedule-decision child; keep the completed decision as a dependency when relevant.
For follow-up batches tied to a pending or approved review task, preserve or infer parent_proposal_id at the workflow-root level and inherit the review date/time only when the message is still about that review step rather than post-event deliverables.
