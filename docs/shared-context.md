...
- Read-only explanation/status questions use validated `direct_responses` and must not mutate proposal or approval state. `[요청]` is an interaction hint, not a task type/title prefix. Mixed answer-plus-mutation decisions are allowed, but each mutation still passes its normal safety gates.
- Ordinary private-DM discussion about tasks, planning, app usage, or draft wording may also use `direct_responses` with no proposal/request target. The `[요청]` marker appears only when the literal tag is present; untagged replies should remain conversational.
- Explicit same-item/merge instructions must use a deterministic duplicate-merge operation, not a correction that leaves `conflict_resolution` pending. The core requires exact source/target IDs, actor authority, compatible hierarchy/date state, and conflict linkage when the source is conflict-held; proposal, approval, decision, and audit mutations are committed together.
- `[요청]` is optional but semantically meaningful: a private tagged message must produce an answer, a supported validated mutation, or a concrete clarification rather than `no_action`. Unknown semantic update keys reject the whole patch so an unsupported operation cannot be silently reduced to a partial title/date edit.

## Change Log

| Date | Actor | Area | What changed | Verification |
| --- | --- | --- | --- | --- |
| 2026-08-05 | Codex | direct answers / Slack request UX | Added a strict state-preserving response branch for explanation, status, reason, and “what should I do?” questions. The core validates private-DM recipient, referenced proposal/request IDs, text, and confidence; mixed answer-plus-patch decisions retain both outputs; a conservative exact-context fallback handles semantic CLI failure without creating work. | Live source commit `3bac967`; clean tests and safety checks recorded in the sync commit. |
| 2026-08-05 | Codex | conversational private DM | Broadened the read-only branch from existing-item explanations to useful task/planning/app conversation. No command tag is required; `[요청]` only controls the optional visual marker, while mutations remain independently validated. | Live source commit `eaccbdb`; clean tests and safety checks recorded in the sync commit. |
| 2026-08-05 | Codex | duplicate merge / conflict resolution | Added `duplicate_merge` as a deterministic semantic mutation. Explicit same-item requests now preserve one canonical proposal, mark the duplicate as rejected merge history, close the source approval, and atomically persist audit state. Legacy merge-shaped confirmation/correction envelopes are promoted to the same operation. | Live source commit `c06634a`; clean tests and safety checks recorded in the sync commit. |
| 2026-08-05 | Codex | explicit request contract | Restored `[요청]` as explicit processing intent while keeping it optional for natural conversation. Tagged private messages cannot disappear as `no_action`, and semantic updates with unknown keys are rejected atomically instead of being partially applied. | Live source commit `fa9fdbc`; clean tests and safety checks recorded in the sync commit. |
